...
;; A* algorithm
(define (A* graph initial node=? hash-node
            node-edges node-cost edge-cost)
  (call/cc
   (lambda (return)
...
(define (display-edge e)
  (format #t "[~A,~A->~A,~A]"
          (map-node:x (map-edge:src e)) (map-node:y (map-edge:src e))
          (map-node:x (map-edge:dst e)) (map-node:y (map-edge:dst e))))
